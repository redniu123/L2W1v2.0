...
def summarize_extended_metrics(
    per_sample: list[dict[str, Any]], boundary_k: int = 2
) -> dict[str, float]:
    boundary_total = 0
    boundary_upgraded = 0
    ...
    for item in per_sample:
        ...
        gt_len_total += len(gt)
    return {
        "Boundary_Deletion_Recall@B": round((boundary_upgraded / boundary_total), 6)
        if boundary_total
        else 0.0,
...
def summarize_latency_and_token_usage(per_sample: list[dict[str, Any]]) -> dict[str, float]:
    latencies = [
        float(item["latency_ms"]) for item in per_sample if item.get("latency_ms") is not None
    ]
    token_usages = [
        float(item["token_usage"]) for item in per_sample if item.get("token_usage") is not None
    ]
    return {
        "P95_Latency_MS": round(float(np.percentile(latencies, 95)), 3) if latencies else 0.0,
        "Avg_Token_Usage": round((sum(token_usages) / len(token_usages)), 3)
        if token_usages
        else 0.0,
