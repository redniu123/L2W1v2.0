...
class CircuitBreaker:
    """Lightweight circuit breaker based on rejection rate."""
    ...
    def observe(self, rejected: bool) -> dict[str, Any]:
        if not self.config.enabled:
        ...
        triggered = False
        if (
            self.total_calls >= self.config.min_samples
            and rejection_rate >= self.config.rejection_rate_threshold
        ):
    ...
    def step_without_call(self) -> dict[str, Any]:
        if not self.config.enabled:
            return self.get_stats()
