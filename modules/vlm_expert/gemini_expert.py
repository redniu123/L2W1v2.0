...
class APIKeyManager:
    ...
    功能：
    - 从 key.txt 读取指定 provider pool 的 API Keys
    - Round-robin 轮询
    - 线程安全（使用 threading.Lock）
    """
    
    def __init__(self, key_file: str = "key.txt", provider_pool: str = "gemini_1x"):
        """
        Args:
            key_file: Key 文件路径（相对于项目根目录）
            provider_pool: provider pool 名称
        """
        ...
        if not self.keys:
            raise ValueError(f"No API keys found for provider pool '{provider_pool}' in {key_file}")
        
        print(f"[APIKeyManager] Loaded {len(self.keys)} API keys from pool '{provider_pool}'")
    
    def _load_keys(self) -> List[str]:
        """从 key.txt 读取指定 provider pool 的 API Keys"""
        pool = get_provider_pool(self.provider_pool, self.key_file)
...
class GeminiConfig:
    ...
    def __post_init__(self):
        # 初始化 APIKeyManager
        self.key_manager = APIKeyManager(self.key_file, provider_pool=self.provider_pool)
        print(f"[GeminiConfig] Using pool={self.provider_pool}, keys={self.key_manager.get_key_count()}")
