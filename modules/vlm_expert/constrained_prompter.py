...
class ConstrainedPrompter:
    ...
    def generate_targeted_correction_prompt(
        ...
        if hint_lines:
            ...
            hint_block = prefix + "\n\n"
        user_prompt = (
            "你是一个极度保守的 OCR 纠错专家，以下是初步的单行文本识别结果：\n"
            + "【 " + T_A + " 】\n\n"
            + hint_block
            + "请结合提供的图像，只在你高度确定存在 OCR 错误时，做最小必要修改。\n"
            + "**最高约束红线：**\n"
            + "1. 除非你能从图像中高度确认 OCR 有误，否则必须逐字原样输出。\n"
            + "2. 绝对禁止润色、扩写、缩写、改写语序、补全你推测但看不清的内容。\n"
            + "3. 绝对禁止仅出于排版美观而修改全半角、括号、句号、冒号、编号样式。\n"
            + "4. 如需修改，只允许做最小必要字符级改动；若拿不准，保持原样。\n\n"
