...
class PipelineResult:
    ...
    def from_dict(cls, data: Dict) -> "PipelineResult":
        ...
        return cls(
            ...
            agent_b_refinement_strategy=agent_b.get(
            ...
            final_text=agent_b.get("text", "")
            if router.get("is_hard")
            else agent_a.get("text", ""),
        )
...
class L2W1Pipeline:
    ...
    def process(
        ...
        if result.agent_a_logits is not None:
            result.agent_a_logits_shape = (
                result.agent_a_logits.shape[-2:]
                if len(result.agent_a_logits.shape) >= 2
                else (0, 0)
            )
    ...
    def _call_agent_b_async(
        ...
        try:
            ...
            if self.config.verbose:
                print(
                    f"[Pipeline] Agent B 超时 (>{self.config.agent_b_timeout_ms}ms)，使用回退"
                )
    ...
    def _log_router_features(
        ...
        log_entry = {
            ...
            "budget_target": budget_details.get(
                "target_budget", self.config.budget_target
            ),
            ...
            if hasattr(scoring_result, "details")
            else {},
        }
    ...
    def _get_environment_info(self) -> str:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if "2080" in gpu_name:
                    return (
                        "RTX2080Ti_4bit_quant"
                        if self.config.agent_b_use_4bit
                        else "RTX2080Ti"
                    )
    ...
    def process_batch(
        self, images: List[Union[str, np.ndarray]], image_paths: List[str] = None
    ) -> List[PipelineResult]:
        """
        批量处理
    ...
    def get_stats(self) -> Dict:
        ...
        if stats["total_processed"] > 0:
            stats["agent_b_routing_rate"] = (
                stats["routed_to_agent_b"] / stats["total_processed"]
            )
            stats["agent_b_correction_rate"] = (
                stats["corrected_by_agent_b"] / stats["routed_to_agent_b"]
                if stats["routed_to_agent_b"] > 0
                else 0.0
            )
            # SH-DA++ v4.0 统计
...
class AgentAAdapter:
    ...
    def __init__(self, args, verbose: bool = False):
        ...
        if verbose:
            print(
                f"[AgentAAdapter] 初始化完成, blank_id={self._recognizer.blank_id}, rho={self._recognizer.rho}"
            )
    ...
    def recognize_batch(self, images: List[Union[str, np.ndarray]]) -> List[Dict]:
        ...
        for i in range(len(img_list)):
            if i < len(results):
                text, confidence = (
                    results[i] if isinstance(results[i], (list, tuple)) else ("", 0.0)
                )
            else:
                text, confidence = "", 0.0
            ...
            output_list.append(
                {
                    ...
                    "confidence": float(confidence) if confidence else 0.0,
                    "logits": None,
                    "boundary_stats": boundary_stats_list[i]
                    if i < len(boundary_stats_list)
                    else {},
                    "top2_info": top2_info_list[i] if i < len(top2_info_list) else {},
                    "lat_router_ms": lat_router_ms_list[i]
                    if i < len(lat_router_ms_list)
                    else 0.0,
                }
            )
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_image):
        ...
        print(f"\n[结果]")
        print(
            f"  Agent A 输出: '{result.agent_a_text}' (置信度: {result.agent_a_confidence:.2%})"
        )
        print(
            f"  Router 决策: is_hard={result.is_hard}, risk_level={result.risk_level}"
        )

        if result.routed_to_agent_b:
            print(
                f"  Agent B 输出: '{result.agent_b_text}' (已修正: {result.agent_b_is_corrected})"
            )
        ...
        print(
            f"  route_type={result.route_type}, lambda_t={result.lambda_t:.4f}, upgrade={result.upgrade}"
        )
    else:
