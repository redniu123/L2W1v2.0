...
功能演示:
1. 初始化 TextRecognizerWithLogits
2. 执行推理并获取原始 logits
3. 验证 logits 形状和熵计算
...
def demo_logits_extraction():
    ...
    print("\n[结论]")
    print(f"  高置信度样本的平均熵 ({np.mean(high_conf_entropy):.4f}) < "
    ...
    try:
        ...
        if hasattr(args, 'image_dir') and os.path.exists(args.image_dir):
            ...
            if img_list:
                ...
                for i, (result, logits) in enumerate(zip(output['results'], output['logits'])):
                    ...
                    if logits is not None:
                        ...
                        print(f"      (Seq_Len={logits.shape[0]}, Vocab_Size={logits.shape[1]})")
                        
                        # 计算熵
                        entropy = compute_visual_entropy(logits)
                        print(f"    - 平均熵: {np.mean(entropy):.4f}")
                        print(f"    - 熵范围: [{np.min(entropy):.4f}, {np.max(entropy):.4f}]")
                    else:
