...
def run_online_routeronly(strategy, target_budget, all_results, agent_b_callable, budget_cfg, run_id='', prompt_version='prompt_v1.1', agent_b_label='configured_agent_b', eta=0.5):
    ...
    for row in tqdm(all_results, desc=f'{strategy} online B={target_budget:.2f}', leave=False):
        ...
        validation_logs.append({'sample_id': item['sample_id'], 'router_name': strategy, 'target_budget': target_budget, 'router_score': q, 'selected_for_upgrade': upgrade, 'budget_details': budget_details, 'run_id': run_id})
...
def main():
    p = argparse.ArgumentParser(description='paper1 RouterOnly online legality check')
    ...
    p.add_argument('--agent_b_skip', action='store_true', default=False)
    p.add_argument('--agent_b_backend', default='local_vlm', choices=['gemini', 'local_vlm'])
    p.add_argument('--agent_b_model_type', default='qwen2.5_vl')
    p.add_argument('--agent_b_model_path', default='./models/agent_b_vlm/qwen3-vl-8b-instruct')
    p.add_argument('--agent_b_torch_dtype', default='float16')
    a = p.parse_args()
    ...
    config['agent_b']['backend'] = a.agent_b_backend
    config['agent_b']['model_type'] = a.agent_b_model_type
    config['agent_b']['model_path'] = a.agent_b_model_path
    config['agent_b']['torch_dtype'] = a.agent_b_torch_dtype
    if a.agent_b_backend == 'local_vlm':
        config['agent_b']['skip'] = False
    if a.agent_b_skip:
    ...
    agent_b_label = 'Qwen3-VL-8B' if a.agent_b_backend == 'local_vlm' else (config.get('mainline_agent_b') or config.get('mainline', {}).get('mainline_agent_b', 'configured_agent_b'))
    ...
    if a.use_cache and not a.rebuild_cache and cache_path.exists():
    ...
    result = run_online_routeronly(a.strategy, a.target_budget, all_results, agent_b_callable, budget_cfg, run_id=run_id, prompt_version=prompt_version, agent_b_label=agent_b_label, eta=eta)

    with (run_dir / 'online_budget_validation.csv').open('w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=list(result['summary'].keys()))
        w.writeheader(); w.writerow(result['summary'])
    ...
    with (run_dir / f"online_budget_{int(round(a.target_budget * 100)):02d}_{a.strategy}.jsonl").open('w', encoding='utf-8') as f:
        for row in result['per_sample']:
            f.write(json.dumps(row, ensure_ascii=False) + '\n')
    (run_dir / 'config_snapshot.yaml').write_text(yaml.safe_dump({'run_id': run_id, 'args': vars(a), 'config': config}, allow_unicode=True, sort_keys=False), encoding='utf-8')
    print(run_dir)
