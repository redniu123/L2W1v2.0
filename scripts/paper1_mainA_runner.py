...
def build_full(agent_rows,agent_b,pv,ab,run_id):
    ...
    def one(r):
        ...
        return {'sample_id':r.get('sample_id',''),'image_path':r.get('image_path',''),'source_image_id':r.get('source_image_id',''),'domain':r.get('domain','geology'),'split':r.get('split','test'),'gt':r['T_GT'],'ocr_text':r['T_A'],'final_text_if_upgraded':up,'final_text':up,'vlm_raw_output':up,'latency_ms':rs.get('latency_ms'),'token_usage':rs.get('token_usage'),'error_type':rs.get('error_type','none'),'has_professional_terms':r.get('has_professional_terms',False),'professional_terms':r.get('professional_terms',[]),'domain_risk_score':round(float(r.get('r_d',0.0)),6),'mean_conf':r.get('mean_conf'),'min_conf':r.get('min_conf'),'drop':r.get('drop'),'conf':r.get('conf'),'r_d':r.get('r_d',0.0),'is_correct_ocr':r['T_A']==r['T_GT'],'edit_distance_ocr':Levenshtein.distance(norm(r['T_A']),norm(r['T_GT'])),'vlm_model':ab,'prompt_version':pv,'run_id':run_id}
    if nw<=1: return [one(r) for r in tqdm(agent_rows,desc='Shared full-call cache')]
    ordered=[None]*len(agent_rows)
    with ThreadPoolExecutor(max_workers=nw) as ex:
        futs={ex.submit(one,r):i for i,r in enumerate(agent_rows)}
        for fut in tqdm(as_completed(futs),total=len(futs),desc=f'Shared full-call cache [{nw} concurrent]'): ordered[futs[fut]]=fut.result()
    return ordered

def replay(strategy,budget,full,score_map,pv,run_id):
    ranked=sorted(range(len(full)),key=lambda i:score_map[i],reverse=True);n=int(round(len(full)*budget));up=set(ranked[:n]);rmap={i:k+1 for k,i in enumerate(ranked)};ps=[];cer=gtl=nup=nacc=0
...
def domain_rows(rows):
    ...
    return [{'router_name':rn,'budget':b,'domain':d,'n_samples':a['n'],'actual_call_rate':round(a['u']/a['n'],4) if a['n'] else 0.0,'CER':round(a['cer']/a['gt'],6) if a['gt'] else 0.0,'final_accuracy':round(a['c']/a['n'],6) if a['n'] else 0.0} for (rn,b,d),a in sorted(acc.items())]
...
def main():
    ...
    pv=a.prompt_version or cfg.get('prompt_version') or cfg.get('mainline',{}).get('prompt_version','prompt_v1.1');ab=cfg.get('agent_b',{}).get('model_name') or cfg.get('mainline_agent_b') or cfg.get('mainline',{}).get('mainline_agent_b','configured_agent_b');eta=float((cfg or {}).get('sh_da_v4',{}).get('rule_scorer',{}).get('eta',0.5));budgets=[float(x) for x in a.budgets.split(',') if x.strip()]
    ...
    samples=[json.loads(x) for x in Path(a.test_jsonl).read_text(encoding='utf-8').splitlines() if x.strip()];out=Path(a.output_dir);out.mkdir(parents=True,exist_ok=True);cache=Path(a.shared_agent_a_cache) if a.shared_agent_a_cache else (out/'shared_agent_a_cache.json')
    if ((a.use_cache or a.shared_agent_a_cache) and not a.rebuild_cache and cache.exists()): ar=ensure_agent_a_result_schema(json.loads(cache.read_text(encoding='utf-8')))
    else: ar=ensure_agent_a_result_schema(infer_all_samples(samples,recog,de,None,a.image_root));cache.write_text(json.dumps(ar,ensure_ascii=False),encoding='utf-8')
