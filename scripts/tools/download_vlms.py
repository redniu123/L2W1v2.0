...
用法:
  python scripts/tools/download_vlms.py --all
  python scripts/tools/download_vlms.py --model qwen2.5_vl
  python scripts/tools/download_vlms.py --model qwen3_vl_8b
  python scripts/tools/download_vlms.py --model internvl2_awq
  python scripts/tools/download_vlms.py --source huggingface --model llava
"""
...
MODEL_REGISTRY = {
    ...
    "qwen3_vl_8b": {
        ...
        "ms_id": "Qwen/Qwen3-VL-8B",
        "local_dir": "Qwen3-VL-8B",
        "desc": "Qwen3-VL-8B (~16GB fp16)",
    },
    "internvl2_5_awq": {
...
def download_modelscope(model_key: str, cfg: dict) -> bool:
    ...
    try:
        snapshot_download(model_id=cfg["ms_id"], local_dir=str(local_dir), ignore_patterns=ignore)
...
def download_huggingface(model_key: str, cfg: dict) -> bool:
    ...
    try:
        snapshot_download(repo_id=cfg["hf_id"], local_dir=str(local_dir), ignore_patterns=ignore)
        print(f"[OK] {model_key}")
        return True
    except Exception as e:
        print(f"[WARN] first attempt failed ({e}), retrying without ignore_patterns...")
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        dest="models",
        help="下载指定模型（可多次指定，如 --model qwen3_vl_8b --model internvl2_awq）",
    )
    ...
    for key, ok in results.items():
        print(f"  {key:20s} {'OK' if ok else 'FAIL'}")
