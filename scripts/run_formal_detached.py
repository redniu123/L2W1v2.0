...
def build_steps(args: argparse.Namespace) -> List[Dict[str, Any]]:
        ...
        steps.append(
            {
                ...
                "command": [
                    ...
                    % str((PROJECT_ROOT / args.batch_output_dir).resolve()).replace("\\", "/")
                ],
            }
        )
    ...
    sanity = [
        ...
        "--n_samples",
        str(args.sanity_samples),
        "--budgets",
        args.batch_budgets,
        "--strategies",
        args.router_strategies,
        "--skip_offline_replay",
    ]
    if args.use_cache:
        sanity.append("--use_cache")
    if args.use_gpu:
    ...
    online = [
        ...
        "--strategy",
        args.system_strategy,
        "--target_budget",
        args.online_budget,
    ]
    if args.use_cache:
        online.append("--use_cache")
    if args.use_gpu:
        online.append("--use_gpu")

    export_command = [
        py,
        "scripts/export_cloud_results.py",
        "--tag",
        args.sync_tag,
        "--source",
        args.batch_output_dir,
        ...
        args.online_output_dir,
        "--source",
        f"{args.automation_output_dir}/{args.run_stamp}",
    ]

    steps.extend(
        [
            {
                "id": "m1_m3_sanity_router_compare",
                "description": f"Sanity router comparison on {args.sanity_samples} samples",
                "shell": False,
                "command": sanity,
            },
            {
                "id": "m1_m3_formal_router_compare",
                "description": "Formal full router comparison run",
                "shell": False,
                "command": formal_batch,
            },
            {
                "id": "m4_online_system_validation",
                "description": f"Online system validation for {args.system_strategy} @ B={args.online_budget}",
                "shell": False,
                "command": online,
            },
            {
                "id": "export_to_cloud_result_sync",
                "description": "Export formal outputs into cloud_result_sync",
                "shell": False,
