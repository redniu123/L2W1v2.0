...
def cache(rows,cfg,bf,rd,pv,shda,path,rebuild):
    ...
    rs=[{'sample_id':x.get('sample_id',''),'image_path':x.get('image_path',''),'ocr_text':x.get('ocr_text',''),'vlm_raw_output':x.get('vlm_raw_output',''),'final_text_if_upgraded':x.get('final_text_if_upgraded',''),'final_text':x.get('final_text',x.get('vlm_raw_output','')),'selected_for_upgrade':x.get('selected_for_upgrade',True),'backfill_status':x.get('backfill_status','skipped' if not shda else 'accepted'),'backfill_reason':x.get('backfill_reason','skip_backfill' if not shda else ''),'circuit_breaker_open':x.get('circuit_breaker_open',False),'circuit_breaker_blocked':x.get('circuit_breaker_blocked',False),'latency_ms':x.get('latency_ms'),'token_usage':x.get('token_usage'),'error_type':x.get('error_type','none'),'vlm_model':x.get('vlm_model',''),'prompt_version':x.get('prompt_version',pv),'cache_variant':'M6' if shda else 'M5'} for x in o['per_sample']];JL(p,rs);return p,rs
def A(rows,label):
    bs={};bp={};bi={};ml=label
    ...
    def call(prompt):
        ...
        fallback=prompt.get('T_A','')
        corrected=((h or {}).get('vlm_raw_output',fallback) or fallback)
        result={'corrected_text':corrected,'latency_ms':0.0,'token_usage':(h or {}).get('token_usage'),'error_type':(h or {}).get('error_type','cache_miss' if h is None else 'none')}
        if h is not None:
            result.update({'cached_final_text':h.get('final_text',corrected),'cached_final_text_if_upgraded':h.get('final_text_if_upgraded',h.get('final_text',corrected)),'cached_backfill_status':h.get('backfill_status','skipped'),'cached_backfill_reason':h.get('backfill_reason',''),'cached_circuit_breaker_open':h.get('circuit_breaker_open',False),'cached_circuit_breaker_blocked':h.get('circuit_breaker_blocked',False)})
        return result
