...
def extract_features_from_output(
    ...
    Args:
        output:        recognizer.__call__ 返回的完整字典
        conf:          整体置信度（来自 results[0][1]）
        T_A:           识别文本
        domain_engine: 语义风险引擎

    Returns:
        features: np.ndarray shape (5,)
    """
    # --- Mean_Confidence & Min_Confidence ---
    # 优先从 top2_info 提取字符级置信度序列
    ...
    if top2_info and isinstance(top2_info, dict):
        char_confs = top2_info.get("char_confs", [])
        if not char_confs:
            char_confs = top2_info.get("top1_probs", [])
    else:
        char_confs = []
...
def process_split(
    ...
    Args:
        ...
        output_prefix: 输出文件前缀（如 results/stage2_v51/features_train）
