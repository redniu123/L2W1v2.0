...
class CollectionStats:
    ...
    # 分诊类型分布
    route_type_counts: Dict[str, int] = field(default_factory=lambda: {
        'none': 0, 'boundary': 0, 'ambiguity': 0, 'both': 0
    })
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "lambda_t_stats": {
                "mean": round(np.mean(self.lambda_t_list), 4) if self.lambda_t_list else 0.0,
                "final": round(self.lambda_t_list[-1], 4) if self.lambda_t_list else 0.0,
            },
        }
...
def run_stage1_collection(
    ...
    for idx, sample in enumerate(tqdm(samples, desc="Processing")):
        try:
            ...
            stats.route_type_counts[result.route_type] = stats.route_type_counts.get(result.route_type, 0) + 1
            ...
            if verbose:
                print(f"[Error] 处理失败 (sample={sample_id if 'sample_id' in dir() else idx}): {e}")
                import traceback
                traceback.print_exc()
            stats.failed_samples += 1
...
def print_summary(stats: CollectionStats):
    ...
    print("│  样本统计                                                           │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print(f"│  总样本数        │  {stats.total_samples:>8}                                      │")
    print(f"│  处理成功        │  {stats.processed_samples:>8}  ({stats.processed_samples/max(stats.total_samples,1)*100:.1f}%)                       │")
    print(f"│  处理失败        │  {stats.failed_samples:>8}  ({stats.failed_samples/max(stats.total_samples,1)*100:.1f}%)                       │")
    print("└─────────────────────────────────────────────────────────────────────┘")
    
    print("\n┌─────────────────────────────────────────────────────────────────────┐")
    print("│  路由决策统计 [核心指标]                                            │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print(f"│  升级样本数      │  {stats.upgrade_count:>8}                                      │")
    print(f"│  upgrade_rate    │  {stats.upgrade_rate:>8.4f}  ({stats.upgrade_rate*100:.2f}%)                    │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print("│  分诊类型分布:                                                      │")
    for route_type, count in sorted(stats.route_type_counts.items()):
        pct = count / max(stats.processed_samples, 1) * 100
        print(f"│    - {route_type:12s}   │  {count:>6}  ({pct:5.1f}%)                           │")
    print("└─────────────────────────────────────────────────────────────────────┘")
    
    print("\n┌─────────────────────────────────────────────────────────────────────┐")
    print("│  耗时统计 [性能指标]                                                │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print(f"│  avg_lat_router_ms │  {stats.avg_lat_router_ms:>8.3f} ms                                │")
    print(f"│  总耗时            │  {stats.total_time_sec:>8.2f} s                                 │")
    if stats.processed_samples > 0:
        throughput = stats.processed_samples / stats.total_time_sec
        print(f"│  吞吐量            │  {throughput:>8.2f} samples/s                         │")
    print("└─────────────────────────────────────────────────────────────────────┘")
    ...
    print("├─────────────────────────────────────────────────────────────────────┤")
    if stats.s_b_list:
        print(f"│  s_b (边界风险)  │  mean={np.mean(stats.s_b_list):.4f}, std={np.std(stats.s_b_list):.4f}           │")
    if stats.s_a_list:
        print(f"│  s_a (歧义风险)  │  mean={np.mean(stats.s_a_list):.4f}, std={np.std(stats.s_a_list):.4f}           │")
    if stats.q_list:
        print(f"│  q (综合优先级)  │  mean={np.mean(stats.q_list):.4f}, p80={np.percentile(stats.q_list, 80):.4f}           │")
    if stats.lambda_t_list:
        print(f"│  λ_t (阈值)      │  init={stats.lambda_t_list[0]:.4f}, final={stats.lambda_t_list[-1]:.4f}          │")
    print("└─────────────────────────────────────────────────────────────────────┘")
...
示例:
    ...
    parser.add_argument("--use_mock", action="store_true",
                        help="使用 Mock 模式（不加载真实模型）")
    parser.add_argument("--verbose", "-v", action="store_true",
                        help="打印详细日志")
    parser.add_argument("--skip_agent_b", action="store_true", default=True,
...
def main():
    ...
    if stats.processed_samples > 0:
        ...
        print(f"  ✓ upgrade_rate: {stats.upgrade_rate:.4f} ({stats.upgrade_rate*100:.2f}%)")
