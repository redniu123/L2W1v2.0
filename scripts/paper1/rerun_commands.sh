...
from scripts.experiments.efficiency_frontier import infer_all_samples, ensure_agent_a_result_schema
...
MAINA_ARGS=("scripts/paper1/mainA_runner.py" --output_dir "$MAINA_OUT" --shared_agent_a_cache "$CACHE_JSON" --use_cache)
if [ "$USE_GPU" = "1" ]; then
  MAINA_ARGS+=(--use_gpu)
fi
python "${MAINA_ARGS[@]}"
...
MAINC_ARGS=("scripts/paper1/mainC_runner.py" --output_dir "$MAINC_OUT" --shared_agent_a_cache "$CACHE_JSON" --use_cache --full_call_cache_dir "$MAINC_OUT")
if [ "$USE_GPU" = "1" ]; then
  MAINC_ARGS+=(--use_gpu)
fi
python "${MAINC_ARGS[@]}"
...
python "scripts/analysis/batch1_analysis.py" --maina_run_dir "$MAINA_RUN" --mainc_run_dir "$MAINC_RUN" --output_root "$PHASE1_OUT"
...
if [ "$SKIP_BATCH2" != "1" ]; then
  ...
  python "scripts/analysis/gpt_case_analysis.py" --maina_run_dir "$MAINA_RUN" --mainc_run_dir "$MAINC_RUN" --output_root "$PHASE2_OUT"
