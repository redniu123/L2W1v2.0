...
import argparse,csv,json,random,time,threading
...
from scripts.experiments.efficiency_frontier import ensure_agent_a_result_schema,infer_all_samples,summarize_extended_metrics,summarize_latency_and_token_usage,build_agent_b_callable
...
def main():
    ...
    recog=TextRecognizerWithLogits(rec);de=DomainKnowledgeEngine({'geology':a.geo_dict,'finance':a.finance_dict,'medicine':a.medicine_dict});samples=[json.loads(x) for x in Path(a.test_jsonl).read_text(encoding='utf-8').splitlines() if x.strip()];out=Path(a.output_dir);out.mkdir(parents=True,exist_ok=True);cache=Path(a.shared_agent_a_cache) if a.shared_agent_a_cache else (out/'shared_agent_a_cache.json')
    if ((a.use_cache or a.shared_agent_a_cache) and not a.rebuild_cache and cache.exists()): rows=ensure_agent_a_result_schema(json.loads(cache.read_text(encoding='utf-8')))
