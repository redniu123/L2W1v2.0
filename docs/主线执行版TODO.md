...
- [ ] 把当前实验策略名从历史命名切换为：
  ...
  - `SH-DA++`
- [ ] 明确每个名字在代码中的逻辑定义
- [ ] 移除正式主实验中的历史名字：`ConfOnly`、`Random`（可保留为 exploratory baseline，但不进主表）
- [ ] 在实验脚本输出 CSV 中同步新名字

**验收标准**：
- 正式脚本跑出来的结果表中，不再出现 `ConfOnly`；
- 所有策略名能直接对应设计书中的定义。

---

### P0-2 拆清 BAUR 与 DAR
**目标**：把“纯不确定性路由”和“领域增强路由”分成两个独立实验分支。

- [ ] `BAUR`：不含 `r_d`
- [ ] `DAR`：在 `BAUR` 基础上加入 `eta * r_d`
- [ ] 给两者加显式代码分支或配置开关
- [ ] 保证输出结果中可以明确区分这两个 Router
...
- [ ] 分数只允许用全局置信度构造，例如：
  - `score = 1 - conf`
  - 或 `score = 1 - mean_conf`
- [ ] 不允许引入 `min_conf`、`drop`、`b_edge`、`r_d`

**验收标准**：
- `GCR` 明显弱于 BAUR，符合论文叙事预期；
- `ConfOnly` 不再被拿来冒充 GCR。

---

### P1-2 明确 BAUR-only 与 SH-DA++ 的系统对照
**目标**：形成 M4 / M5 主实验。

- [ ] `BAUR-only`：只保留 BAUR 路由主链
- [ ] `SH-DA++`：BAUR + 领域软提示 + 严格回填 + 熔断
- [ ] 写清楚两者唯一变量差异

**验收标准**：
- `BAUR-only` 和 `SH-DA++` 能在统一脚本中并列输出；
- 论文中可以直接引用这两个实验编号。
