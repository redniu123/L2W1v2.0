...
- ✅ 进度更新频率为每 1000 个样本
- ✅ `router_features.jsonl` 中包含 `agent_a_text` 与 `v_edge_raw`
...
# 检查 v_edge_raw 是否写入
head -1 results/router_features.jsonl | jq '.v_edge_raw'
```
...
- `results/metrics_summary.json` - 评估指标汇总

**验证命令**：

```bash
# 查看核心指标
cat results/metrics_summary.json | jq '.boundary_audit.boundary_deletion_recall_at_b'
cat results/metrics_summary.json | jq '.cer_improvement'
```
